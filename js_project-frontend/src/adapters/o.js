...
function flappyDotStart() {
  document.getElementById('flappy-dot').addEventListener('click', (e) => {
    ...
    document.getElementById('navbarToggleExternalContent').className =
      'collapse';
    startFlappyGame();
  });
}
...
function startFlappyGame() {
  ...
  function handleCollision() {
    for (let i = 0; i < obstaclesArray.length; i++) {
      ...
      if (
        ...
        ctx.fillStyle = 'red';
        ctx.fillText(
          `HAHA WHAT A LOOSER! SCORED ONLY: ${score}`,
          60,
          canvas.height / 2
        );
        return true;
  ...
  function finishGame() {
    ...
    setTimeout(function () {
      canvas.remove();
      document
        .getElementById('carousel-parent')
        .insertAdjacentHTML('afterend', scoreForm);
      document
        .getElementById('score-button')
        .insertAdjacentHTML('afterend', scoreTable);
      fetch(`${domain}/api/flappy_dot/flappy_dot_top_10_players`)
        .then((object) => object.json())
        .then((object) => fillScores(object));
  ...
  function submitScore() {
    document
      ...
      .addEventListener('submit', function (e) {
        const name = document.getElementById('submit-score').childNodes[1]
          .value;
